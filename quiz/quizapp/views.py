...
def single_quiz(request, quiz_id):
    ...
    return render(request, 'quizapp/single_quiz.html', context)
...
def single_question(request, quiz_id, question_id):
    ...
    return render(request, 'quizapp/single_question.html', context)
...
def vote(request, quiz_id, question_id):
    ...
    try:
        ...
        return render(request, 'quizapp/single_question.html', {
            'quiz': quiz,
            'current_question': current_question,
        ...
        if selected_choice == correct_answer:
            print("You are right")
            request.session["num_correct"] += 10
        else:
            print("You are wrong")
            request.session["num_wrong"] += 10

        # checks if next page should be results or next question
        if question_id == (len(quiz.question_set.all())):
            return HttpResponseRedirect(reverse('quizapp:results', args=(quiz.id,)))
        else :
            return HttpResponseRedirect(reverse('quizapp:single_question', args=(quiz.id, question_id+1,)))
...
def create_quiz(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(reverse('quizapp:create_question', args=(new_quiz.id, 1,)))
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if question_id == quiz.num_questions:
                return HttpResponseRedirect(reverse('quizapp:index'))
            else:
                return HttpResponseRedirect(reverse('quizapp:create_question', args=(quiz_id, question_id+1,)))
